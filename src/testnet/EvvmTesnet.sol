...
import {EvvmTesnetStorage} from "Tesnet-contracts/EvvmTesnetStorage.sol";

contract EvvmTesnet is EvvmTesnetStorage {
    modifier onlyAdmin() {
    ...
    constructor(address _initialOwner, address _sMateContractAddress) {
        ...
        MateNameServiceTesnet mateNameService = new MateNameServiceTesnet(
            address(this),
            _initialOwner
        );
    ...
    function payNoMateStaking_sync(
        ...
        address to = !Strings.equal(to_identity, "")
            ? MateNameServiceTesnet(mateNameServiceAddress)
        ...
        for (uint256 i = 0; i < toData.length; i++) {
            ...
            if (!Strings.equal(toData[i].to_identity, "")) {
                if (
                    ...
                    to_aux = MateNameServiceTesnet(mateNameServiceAddress)
