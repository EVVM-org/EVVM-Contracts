...
contract EstimatorTesnet {
        ...
        )
    {
        ...
        */
        ...
        SMateTesnet.HistoryMetadata memory h;
        uint256 size = SMateTesnet(addressSMate.actual).getSizeOfAddressHistory(
            _user
        );

        for (uint256 i = 0; i < size; i++) {
            h = SMateTesnet(addressSMate.actual).getAddressHistoryByIndex(
                _user,
                i
            );
