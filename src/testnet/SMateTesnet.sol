...
import {MateNameServiceTesnet} from "Tesnet-contracts/MateNameServiceTesnet.sol";
import {EstimatorTesnet} from "Tesnet-contracts/EstimatorTesnet.sol";

contract SMateTesnet {
    error Time(uint256);
    ...
    constructor(address initialAdmin) {
        ...
        EvvmTesnet evvm = new EvvmTesnet(initialAdmin, address(this));
    ...
    function goldenStaking(
        ...
        stakingProcess(
            ...
            _amountOfSMate,
            0,
            EvvmTesnet(EVVM_ADDRESS).getNextCurrentSyncNonce(msg.sender),
            false,
            _signature_Evvm
        );
    }
    ...
    */
    ...
    function stakingProcess(
        ...
        if (_isStaking) {
            ...
            EvvmTesnet(EVVM_ADDRESS).pointStaker(_user, 0x01);
            ...
            */
            ...
            if (_amountOfSMate == getUserAmountStaked(_user)) {
                ...
                EvvmTesnet(EVVM_ADDRESS).pointStaker(_user, 0x00);
            }
        ...
        if (EvvmTesnet(EVVM_ADDRESS).isMateStaker(msg.sender)) {
            ///@dev give the reward to the fisher
            makeCaPay(
                MATE_TOKEN_ADDRESS,
                msg.sender,
                (EvvmTesnet(EVVM_ADDRESS).seeMateReward() * 2) + _priorityFee_Evvm
            );
        ...
        )
    {
        if (userHistory[user].length > 0) {
            ...
            ) = EstimatorTesnet(estimator.actual).makeEstimation(user);
            ...
            if (amountTotalToBeRewarded > 0) {
                ...
                if (EvvmTesnet(EVVM_ADDRESS).isMateStaker(msg.sender)) {
                    makeCaPay(
                        MATE_TOKEN_ADDRESS,
                        msg.sender,
                        (EvvmTesnet(EVVM_ADDRESS).seeMateReward() * 1)
                    );
                }
            }
        }
    ...
    function makePay(
        ...
        if (_priority_Evvm) {
            EvvmTesnet(EVVM_ADDRESS).payMateStaking_async(
                _user_Evvm,
                address(this),
                "",
                MATE_TOKEN_ADDRESS,
            ...
            EvvmTesnet(EVVM_ADDRESS).payMateStaking_sync(
                _user_Evvm,
                address(this),
                "",
                MATE_TOKEN_ADDRESS,
