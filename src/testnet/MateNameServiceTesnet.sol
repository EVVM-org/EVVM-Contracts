...
contract MateNameServiceTesnet {
    ...
    function preRegistrationUsername(
        ...
        if (EvvmTesnet(evvmAddress.current).isMateStaker(msg.sender)) {
            makeCaPay(
                msg.sender,
                EvvmTesnet(evvmAddress.current).seeMateReward() +
                    _priorityFeeForFisher
            );
        }
    }
    ...
    function registrationUsername(
        ...
        if (EvvmTesnet(evvmAddress.current).isMateStaker(msg.sender)) {
            makeCaPay(
                msg.sender,
                (50 * EvvmTesnet(evvmAddress.current).seeMateReward()) +
                    _priorityFeeForFisher
            );
        }
    ...
    function registrationTelephoneNumber(
        ...
        if (EvvmTesnet(evvmAddress.current).isMateStaker(msg.sender)) {
            makeCaPay(
                msg.sender,
                (multiple * EvvmTesnet(evvmAddress.current).seeMateReward()) +
                    _priorityFeeForFisher
            );
        }
    ...
    */
        ...
        verifyIfNonceIsAvailable(_user, _nonce)
    {
        ...
        if (EvvmTesnet(evvmAddress.current).isMateStaker(msg.sender)) {
            makeCaPay(
                msg.sender,
                (5 * EvvmTesnet(evvmAddress.current).seeMateReward()) +
    ...
    function proposeWithdrawMateTokens(uint256 _amount) public onlyAdmin {
        if (
            EvvmTesnet(evvmAddress.current).seeBalance(
                address(this),
                MATE_TOKEN
            ) -
                (5083 +
    ...
    function makePay(
        ...
        if (_priority_Evvm) {
            EvvmTesnet(evvmAddress.current).payMateStaking_async(
                _user_Evvm,
                address(this),
                "",
                MATE_TOKEN,
            ...
            EvvmTesnet(evvmAddress.current).payMateStaking_sync(
                _user_Evvm,
                address(this),
                "",
                MATE_TOKEN,
    ...
    function makeCaPay(address _user_Evvm, uint256 _ammount_Evvm) internal {
        EvvmTesnet(evvmAddress.current).caPay(
            _user_Evvm,
            MATE_TOKEN,
            _ammount_Evvm
        );
    ...
    function seePriceToRenew(
        ...
        if (identityDetails[_identity].expireDate >= block.timestamp) {
            ...
            if (price == 0) {
                ...
                uint256 mateReward = EvvmTesnet(evvmAddress.current)
                    .seeMateReward();
            ...
            price = 500_000 * EvvmTesnet(evvmAddress.current).seeMateReward();
        }
    }

    function getPriceToAddCustomMetadata() public view returns (uint256 price) {
        price = 10 * EvvmTesnet(evvmAddress.current).seeMateReward();
    }
    ...
    function getPriceToFlushCustomMetadata(
        ...
        price =
            (10 * EvvmTesnet(evvmAddress.current).seeMateReward()) *
            identityDetails[_identity].customMetadataMaxSlots;
    }
    ...
    function getPricePerRegistration() public view returns (uint256) {
        return EvvmTesnet(evvmAddress.current).seeMateReward() * 100;
    }
