...
import {EvvmTesnetStructs} from "Tesnet-contracts/EvvmTesnetStructs.sol";
...
contract P2PSwap {
    ...
    function makeOrder(
        ...
        if (EvvmTesnet(evvmAddress).isMateStaker(msg.sender)) {
            ...
            makeCaPay(
                ...
                MATE_TOKEN_ADDRESS,
                _priorityFee_Evvm > 0
                    ? (EvvmTesnet(evvmAddress).seeMateReward() * 3)
                    : (EvvmTesnet(evvmAddress).seeMateReward() * 2)
            );
        }
    ...
    function dispatchOrder_fillPropotionalFee(
        ...
        EvvmTesnetStructs.DisperseCaPayMetadata[]
            memory toData = new EvvmTesnetStructs.DisperseCaPayMetadata[](2);

        toData[0] = EvvmTesnetStructs.DisperseCaPayMetadata(
            ordersInsideMarket[market][metadata.orderId].amountB +
                (fee * (rewardPersentage.seller / 10_000)),
            ordersInsideMarket[market][metadata.orderId].seller
        );
        toData[1] = EvvmTesnetStructs.DisperseCaPayMetadata(
        ...
        if (EvvmTesnet(evvmAddress).isMateStaker(msg.sender)) {
            makeCaPay(
                ...
                metadata.amountOfTokenBToFill >
                    ordersInsideMarket[market][metadata.orderId].amountB + fee
                    ? EvvmTesnet(evvmAddress).seeMateReward() * 5
                    : EvvmTesnet(evvmAddress).seeMateReward() * 4
            );
        }
    ...
    function calculateFillFixedFee(
        ...
        if (token != ETH_ADDRESS) {
            if (
                EvvmTesnet(evvmAddress).getTokenUniswapPool(token) == address(0)
            ) {
    ...
    function makePay(
        ...
        if (_priority_Evvm) {
            EvvmTesnet(evvmAddress).payMateStaking_async(
                _user_Evvm,
                address(this),
                "",
                _token_Evvm,
            ...
            EvvmTesnet(evvmAddress).payMateStaking_sync(
                _user_Evvm,
                address(this),
                "",
                _token_Evvm,
    ...
    function makeCaPay(
        ...
        EvvmTesnet(evvmAddress).caPay(_user_Evvm, _token_Evvm, _ammount_Evvm);
    }

    function makeDisperseCaPay(
        EvvmTesnetStructs.DisperseCaPayMetadata[] memory toData,
        address token,
        uint256 amount
    ) internal {
        EvvmTesnet(evvmAddress).disperseCaPay(toData, token, amount);
    }
