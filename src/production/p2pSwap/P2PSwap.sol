...
/**
8""""8 eeee 8""""8    8""""8                      
8    8    8 8    8    8      e   e  e eeeee eeeee 
8eeee8    8 8eeee8    8eeeee 8   8  8 8   8 8   8 
88     eee8 88            88 8e  8  8 8eee8 8eee8 
88     8    88        e   88 88  8  8 88  8 88    
88     8eee 88        8eee88 88ee8ee8 88  8 88    
...
contract P2PSwap {
    ...
    function dispatchOrder_fillPropotionalFee(
        ...
        Evvm.DisperseCaPayMetadata[]
            memory toData = new Evvm.DisperseCaPayMetadata[](2);

        toData[0] = Evvm.DisperseCaPayMetadata(
            ordersInsideMarket[market][metadata.orderId].amountB +
                (fee * (rewardPersentage.seller / 10_000)),
            ordersInsideMarket[market][metadata.orderId].seller
        );
        toData[1] = Evvm.DisperseCaPayMetadata(
    ...
    function calculateFillFixedFee(
        ...
        if (token != ETH_ADDRESS) {
            if (Evvm(evvmAddress).getTokenUniswapPool(token) == address(0)) {
                fee = calculateFillPropotionalFee(amount);
            } else {
                uint256 amountOut = Evvm(evvmAddress)
                    .estimateEthToTokenUsingUniswapV3Pool(
                        Evvm(evvmAddress).getTokenUniswapPool(token),
                        token,
                        uint128(maxLimitFillFixedFee),
                        3000
                    );
                if (calculateFillPropotionalFee(amount) > amountOut) {
                    fee = amountOut;
    ...
    function makeDisperseCaPay(
        Evvm.DisperseCaPayMetadata[] memory toData,
        address token,
        uint256 amount
    ) internal {
