...
import {IPhoneNumberRegistery} from "@RollAMate/interfaces/mateNameService/IPhoneNumberRegistery.sol";
import {IEmailRegistery} from "@RollAMate/interfaces/mateNameService/IEmailRegistery.sol";
...
contract MateNameService {
    ...
    modifier onlyAdminOfIdentity(address _user, string memory _identity) {
    ...
    function registrationUsername(
        ...
        bool _priority_Evvm,
        bytes memory _signature_Evvm
    ) external verifyIfNonceIsAvailable(_user, _nonce) {
        if (admin.current != _user) {
            isValidUsername(_username);
        }

        if (
            !isUsernameAvailable(_username) ||
    ...
    /**
     *  @notice This function is used to register a telephone number
     *  @param _user the address of the user who wants to register
     *  @param _nonce the nonce of the user
     *  @param _phoneNumber the telephone number to register
     *  @param _timestampUser the timestamp of the registration of the user
     *  @param _signatureUser the signature of the registration of the user
     *  @param _timestampAuthority the timestamp of the registration of the authority
     *  @param _signatureAuthority the signature of the registration of the authority
     *  @param _priorityFeeForFisher the priority fee for the fisher who will include
     *  @param _nonce_Evvm the nonce of the user in the Evvm
     *  @param _priority_Evvm the priority of the transaction in the
     *  @param _signature_Evvm the signature of the transaction in the
    ...
    /**
     *  @notice This function is used to register an email
     *  @param _user the address of the user who wants to register
     *  @param _nonce the nonce of the user
     *  @param _email the email to register
     *  @param _timestampUser the timestamp of the registration of the user
     *  @param _signatureUser the signature of the registration of the user
     *  @param _timestampAuthority the timestamp of the registration of the authority
     *  @param _signatureAuthority the signature of the registration of the authority
    ...
    /**
     *  @notice This function is used to make an offer for a username
     *  @param _user the address of the user who wants to make the offer
     *  @param _nonce the nonce of the user
     *  @param _username the username to make the offer
     *  @param _amount the amount of MATE to offer plus 0.5% of the amount
     *                 for taxes
     *  @param _expireDate the date of expiration of the offer in unix time
    ...
    /**
     *  @notice This function is used to withdraw an offer for a username
     *  @param _user the address of the user who wants to withdraw the offer
     *  @param _nonce the nonce of the user
     *  @param _username the username to withdraw the offer
    ...
    function acceptOffer(
        ...
        public
        onlyAdminOfIdentity(_user, _username)
        verifyIfNonceIsAvailable(_user, _nonce)
    {
        if (
            usernameOffers[_username][_offerID].offerer == address(0) ||
    ...
    */
    ...
    /**
     *  @notice This function is used to remove a custom metadata from an identity
     *  @param _user address of user who wants to remove the custom metadata
     *  @param _nonce nonce in the mateNameServiceNonce
     *  @param _identity the identity to remove the custom metadata
     *  @param _key the key of the custom metadata to remove
     *  @param _priorityFeeForFisher the priority fee for the fisher who will include the
     *                               transaction
     *  @param _signature the signature of the transaction
     *  @param _nonce_Evvm nonce in the Evvm
     *  @param _priority_Evvm the priority of the transaction in the
     *                        Evvm's payMateStaker function
     *  @param _signature_Evvm the signature of the transaction in the
     *                         payMateStaker function in the Evvm
    ...
    /**
     *  @notice This function is used to flush all custom metadata from an identity
     *  @param _user address of user who wants to flush the custom metadata
     *  @param _nonce nonce in the mateNameServiceNonce
     *  @param _identity the identity to flush the custom metadata
     *  @param _priorityFeeForFisher the priority fee for the fisher who will include the
     *                               transaction
     *  @param _signature the signature of the transaction
     *  @param _nonce_Evvm nonce in the Evvm
    ...
    /*
        funcion que sea flushName permisonless que permita eliminar 
        completamente el mapping de un nombre a 0 y hace flush 
        metadata, la condicion es que el nombre este vencido
     */
    ...
    function seePriceToRenew(
        ...
        if (identityDetails[_identity].expireDate >= block.timestamp) {
            ...
            if (price == 0) {
                ...
                price = ((price * 5) / 1000) >
                    (500000 * Evvm(evvmAddress.current).seeMateReward())
                    ? (500000 * Evvm(evvmAddress.current).seeMateReward())
                    : ((price * 5) / 1000);
            }
        } else {
