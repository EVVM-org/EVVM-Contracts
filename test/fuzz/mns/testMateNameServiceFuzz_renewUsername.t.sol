...
contract testMateNameServiceFuzz_renewUsername is Test {
    SMateMock sMate;
    EvvmMock evvm;
    ...
    modifier prepareUsernameAndSetOffers(uint256 seed) {
        address user;
        uint8 v;
        bytes32 r;
        bytes32 s;
        ...
        uint256 offer1 = bound(
            seed,
            500000 * evvm.seeMateReward(),
            (500000 * evvm.seeMateReward()) * 10
        );

        uint16 nonce1 = uint16(bound(seed, 70000, 100000));

        evvm._addBalance(USER2, MATE_TOKEN_ADDRESS, offer1);

        (v, r, s) = vm.sign(
            USER2_PRIVATE_KEY,
            Erc191TestBuilder.buildMessageSignedForMakeOffer(
                "test",
                4121818753, //set to Fri Aug 13 2100 05:39:13 GMT+0000
                offer1,
                nonce1
            )
        );
    ...
    function test__fuzz__renewUsername__async__nPF__nOffers(
        uint32 nonceMNS,
        uint32 nonceEVVM
    ) external prepareUsername {
        nonceMNS = uint32(bound(uint256(nonceMNS), 20000, 60000));
        nonceEVVM = uint32(bound(uint256(nonceEVVM), 5000, 10000));

        uint256 newUsernameExpirationTime;
        uint256 oldUsernameExpirationTime;
        uint8 v;
        bytes32 r;
        bytes32 s;
        bytes memory signatureMNS;
        ...
        (, oldUsernameExpirationTime) = mns.getIdentityBasicMetadata("test");
        ...
        (, newUsernameExpirationTime) = mns.getIdentityBasicMetadata("test");

        assert(newUsernameExpirationTime > oldUsernameExpirationTime);
    }

    function test__fuzz__renewUsername__sync__nPF__nOffers(
        uint32 nonceMNS,
        uint32 nonceEVVM
    ) external prepareUsername {
        nonceMNS = uint32(bound(uint256(nonceMNS), 20000, 60000));
    ...
    function test__fuzz__renewUsername__async__PF__nOffers(
        uint32 nonceMNS,
        uint32 nonceEVVM,
        uint72 priorityFee
    ) external prepareUsername {
        ...
        evvm._addBalance(
            USER1,
            MATE_TOKEN_ADDRESS,
            mns.seePriceToRenew("test") + uint256(priorityFee)
        );
    ...
    function test__fuzz__renewUsername__sync__PF__nOffers(
        uint32 nonceMNS,
        uint32 nonceEVVM,
        uint72 priorityFee
    ) external prepareUsername {
    ...
    function test__fuzz__renewUsername__async__nPF__Offers(
        uint32 nonceMNS,
        uint32 nonceEVVM,
        uint32 seed
    ) external prepareUsernameAndSetOffers(seed) {
        nonceMNS = uint32(bound(uint256(nonceMNS), 20000, 60000));
        nonceEVVM = uint32(bound(uint256(nonceEVVM), 5000, 10000));
    ...
    function test__fuzz__renewUsername__sync__nPF__Offers(
        uint32 nonceMNS,
        uint32 nonceEVVM,
        uint32 seed
    ) external prepareUsernameAndSetOffers(seed) {
    ...
    function test__fuzz__renewUsername__async__PF__Offers(
        uint32 nonceMNS,
        uint32 nonceEVVM,
        uint72 priorityFee,
        uint32 seed
    ...
    function test__fuzz__renewUsername__sync__PF__Offers(
        uint32 nonceMNS,
        uint32 nonceEVVM,
        uint72 priorityFee,
        uint32 seed
