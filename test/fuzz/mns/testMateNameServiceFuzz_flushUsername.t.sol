...
contract testMateNameServiceFuzz_flushUsername is Test {
    SMateMock sMate;
    EvvmMock evvm;
    ...
    modifier prepareUsernameAndCustomMetadata(uint256 seed) {
        ...
        uint256 maxCustomMetadata = bound(seed, 0, 20);
    ...
    function test__fuzz__removeCustomMetadata__async__nPF(
        ...
        bytes memory signatureEVVM;
        address user;
        uint256 priceFlushName;

        priceFlushName = mns.getPriceToFlushUsername("test");

        evvm._addBalance(USER1, MATE_TOKEN_ADDRESS, priceFlushName);
    ...
    function test__fuzz__removeCustomMetadata__async__PF(
        ...
        evvm._addBalance(
            USER1,
            MATE_TOKEN_ADDRESS,
            priceFlushName + uint256(priorityFee)
        );
