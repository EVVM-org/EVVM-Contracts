...
contract testMateNameServiceFuzz_flushCustomMetadata is Test {
    SMateMock sMate;
    EvvmMock evvm;
    ...
    modifier prepareUsernameAndCustomMetadata(uint256 seed) {
        ...
        uint256 maxCustomMetadata = bound(seed, 1, 20);

        evvm._addBalance(
            USER1,
            MATE_TOKEN_ADDRESS,
            ((10 * evvm.seeMateReward()) * maxCustomMetadata + 1) +
        ...
        for (uint i = 0; i < maxCustomMetadata; i++) {
    ...
    function test__fuzz__removeCustomMetadata__async__nPF(
        ...
        nonceEVVM = uint32(bound(seed, 10_000, 99_000));
        nonceMNS = uint32(bound(seed, 10_000, 99_000));
        ...
        evvm._addBalance(USER1, MATE_TOKEN_ADDRESS, priceFlushCustomMetadata);
        ...
        uint256 amountOfCustomMetadataSlots = mns.getAmountOfCustomMetadata(
            "test"
        );

        assertEq(amountOfCustomMetadataSlots, 0);
    }
    ...
    function test__fuzz__removeCustomMetadata__async__PF(
        ...
        evvm._addBalance(
            USER1,
            MATE_TOKEN_ADDRESS,
            priceFlushCustomMetadata + uint256(priorityFee)
        );
