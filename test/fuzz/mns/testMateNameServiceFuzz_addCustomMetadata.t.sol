...
contract testMateNameServiceFuzz_addCustomMetadata is Test {
    SMateMock sMate;
    EvvmMock evvm;
    ...
    function test__fuzz__addCustomMetadata__async__nPF(
        uint16 nonceMNS,
        uint16 nonceEVVM,
        uint16 seed
    ...
    function test__fuzz__addCustomMetadata__sync__nPF(
        uint16 nonceMNS,
        uint16 nonceEVVM,
        uint16 seed
    ) external prepareUsername {
    ...
    function test__fuzz__addCustomMetadata__async__PF(
        uint16 nonceMNS,
        uint16 nonceEVVM,
        uint16 seed,
        uint40 priorityFee
        ...
        nonceMNS = uint16(bound(nonceMNS, 1000, type(uint16).max - 10));
        nonceEVVM = uint16(bound(nonceEVVM, 1000, type(uint16).max - 10));
        priorityFee = uint40(bound(priorityFee, 5, type(uint40).max - 10));
        ...
        evvm._addBalance(
            USER1,
            MATE_TOKEN_ADDRESS,
            ((10 * evvm.seeMateReward()) + uint256(priorityFee)) * 5
        );
    ...
    function test__fuzz__addCustomMetadata__sync__PF(
        uint16 nonceMNS,
        uint16 nonceEVVM,
        uint16 seed,
        uint40 priorityFee
